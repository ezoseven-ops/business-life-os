  ...
  return (
    <div className="min-h-dvh" style={{ backgroundColor: 'var(--color-cc-bg)' }}>
      <Header
        title="Calendar"
        action={
          <Link prefetch={false} href="/calendar/new" className="text-sm font-semibold text-[var(--color-cc-accent)]">
            + New
          </Link>
        }
      />
      ...
      <div className="px-4 py-4 max-w-lg mx-auto pb-24">
        {events.length === 0 ? (
          <EmptyState
            ...
            action={
              <Link prefetch={false} href="/calendar/new" className="px-5 py-2.5 bg-[var(--color-cc-accent)] text-[var(--color-cc-bg)] rounded-xl text-sm font-semibold">
                New event
              </Link>
            }
          />
          ...
          <div className="space-y-5">
            {grouped.map(([dateLabel, dateEvents]) => (
              <div key={dateLabel}>
                <h3 className="text-xs font-semibold uppercase tracking-wider mb-2 px-1" style={{ color: 'var(--color-cc-text-muted)' }}>
                  {dateLabel}
                </h3>
                  ...
                  {dateEvents.map((event: Record<string, unknown>) => (
                    ...
                    >
                      <div className="flex items-start gap-3">
                        <div className="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 mt-0.5" style={{ backgroundColor: 'var(--color-cc-accent-glow)' }}>
                          <svg className="w-4 h-4 text-[var(--color-cc-accent)]" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-2 mt-1 text-xs" style={{ color: 'var(--color-cc-text-muted)' }}>
